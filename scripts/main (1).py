...
_SUFFIX = "" if config.LANG == "ar" else f"_{config.LANG}"
QUEUE = config.ROOT / "topics" / f"queue{_SUFFIX}.txt"
DONE = config.ROOT / "topics" / f"done{_SUFFIX}.txt"
...
def next_topic() -> str:
    if QUEUE.exists():
        ...
        if lines:
            return lines[0]
    if config.LANG == "en":
        return "One quick tip to burn belly fat at home with no equipment"
...
def main():
    ...
    if config.SITE not in desc:
        if config.LANG == "en":
            desc += f"\n\nFull training programs: https://{config.SITE}"
        else:
            desc += f"\n\nمزيد من البرامج والكتب: https://{config.SITE}"
    if fmt == "shorts" and config.LANG == "ar":
